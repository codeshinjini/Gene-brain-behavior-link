...
function [ f1t,f2t,f3t,I0_recon,Ierror,flag ] = compVOTGradients( f1,f2,f3,I0,I1,lambda )
%Computes the gradients required in Kundu et al's variation optimization approach
...
It=abs(interp3(I1,f1,f2,f3,'linear',min(I1(:))));     %linear interpolation does not produce unwanted negative values, cubic
Ierror=detf.*It-I0;
...
f1t=detf.*Itx.*Ierror-divD1 + lambda*curlC1;
f2t=detf.*Ity.*Ierror-divD2 + lambda*curlC2;
f3t=detf.*Itz.*Ierror-divD3 + lambda*curlC3;
